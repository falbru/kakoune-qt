...
void SplitContainer::split(QWidget *source_widget, QWidget *new_widget, Qt::Orientation orientation)
{
    ...
    // NOTE: replaceWidget() will move focus to another widget, which will screw up the focus order in LastFocusedFilter. We therefore clear the focus before calling the function, and reapply the focus after.
    ...
    if (regain_focus_widget) regain_focus_widget->setFocus();

    // NOTE: Adding the source_widget to the container after calling replaceWidget() will ensure that the splitter ratio will be preserved.
