...
KakouneInfoBox::KakouneInfoBox(KakouneClient *client, KakouneMenu *menu, DrawOptions *draw_options, QWidget *parent) : QWidget(parent), m_client(client), m_menu(menu), m_draw_options(draw_options) {
...
void KakouneInfoBox::applyInlineStyle(InlineType type) {
  ...
  QPoint anchor_position(anchor.column * m_draw_options->getCellSize().width(), anchor.line * m_draw_options->getCellSize().height());
...
void KakouneInfoBox::paintEvent(QPaintEvent *ev)
{
    ...
    if (title.contentSize() > 0) {
      title.draw(context, QPoint((width() - title.contentSize() * m_draw_options->getCellSize().width())/2, 0), m_client->getInfoFace());
    }
