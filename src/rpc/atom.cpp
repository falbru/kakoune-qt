...
namespace RPC {
    Atom::Atom(Face face, QString contents)
        : m_face(face), m_contents(contents)
    {
    ...
    void Atom::draw(const DrawContext& context, const QPoint& position, const Face& default_face) {
        ...
        context.painter.fillRect(position.x(), position.y(), context.cell_size.width() * m_contents.size(), context.cell_size.height(), bg);
